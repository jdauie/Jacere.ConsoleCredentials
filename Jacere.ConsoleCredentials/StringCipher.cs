...
namespace Jacere.ConsoleCredentials
{
    ...
    public static class StringCipher
    {
        ...
        private const CipherMode KeyCipherMode = CipherMode.CBC;
        private const PaddingMode KeyPaddingMode = PaddingMode.PKCS7;

        public static string Encrypt(string plainText, string passPhrase)
        {
            // Salt and IV is randomly generated each time, but is preprended to encrypted cipher text
            // so that the same Salt and IV values can be used when decrypting.  
            var saltStringBytes = GenerateRandomEntropy(KeySize);
            ...
            using (var password = new Rfc2898DeriveBytes(passPhrase, saltStringBytes, DerivationIterations))
            {
                ...
                using (var symmetricKey = new RijndaelManaged())
                {
                    ...
                    symmetricKey.Mode = KeyCipherMode;
                    symmetricKey.Padding = KeyPaddingMode;
        ...
        {
            var randomBytes = new byte[bits / 8];
